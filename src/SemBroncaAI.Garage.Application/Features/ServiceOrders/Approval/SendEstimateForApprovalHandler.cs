...
{
    public async Task<ApprovalSummaryResponse> HandleAsync(Guid id, CancellationToken cancellationToken = default)
    {
        ...
        var approval = order.SendForApproval(token.Hash, token.ProtectedValue, now.AddDays(7), now);
