...
public sealed class TeamService(HttpClient client)
{
    ...
    public async Task<TeamOperationResult> ResendInvitationAsync(Guid id) => await Send(await client.PostAsync($"api/team/{id}/resend-invitation", null));
...
{
    public async Task<bool> CanAcceptAsync(Guid invitationId, string token)
    {
        using var response = await client.GetAsync($"api/team-invitations/validate?id={invitationId:D}&token={Uri.EscapeDataString(token)}");
