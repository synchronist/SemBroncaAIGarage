...
public static class AuthorizedLandingPage
{
    public static string For(ClaimsPrincipal user)
    {
        ...
        if (Has(user, ApplicationPermissions.ViewServiceOrders)) return "/dashboard";
    ...
    private static bool CanAccess(ClaimsPrincipal user, string path)
    {
        ...
        if (path.Equals("/dashboard", StringComparison.OrdinalIgnoreCase)) return Has(user, ApplicationPermissions.ViewServiceOrders);
