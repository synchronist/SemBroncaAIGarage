...
public sealed record PublicApprovalModel(string GarageName, string? GaragePhone, string? GarageEmail,
    ...
    IReadOnlyCollection<PublicApprovalItemModel> Items);
public sealed record PublicApprovalItemModel(string Description, string Type, decimal Quantity,
    decimal UnitPrice, decimal Total);
public sealed record PublicApprovalDecision(string? CustomerName, string? Comment);
